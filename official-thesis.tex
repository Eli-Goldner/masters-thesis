...
\program{Graduate Program in Computational Linguistics\\Department of Computer Science}
...
\begin{acknowledgements}
  ...
  the approach to the thesis, and Sophia Malamud for her helpful comments and questions.
...
\par
AMR extensions ameliorate core AMR's lack of expressivity by integrating more semantic information
...
curiosity.  For robust and effective natural language understanding,
it is arguable that a semantic representation needs to deeply reflect natural language at the expressive level
while supporting complex reasoning and capturing both world and domain-specific knowledge \citep{KK_Schubert2015}.
While statistically driven methods have made definitive contributions to tasks in deep understanding such as dialogue, question answering,
natural language inference, and textual entailment, semantic tasks relevant to these such as
disambiguation of discourse sentences \citep{Gardent2001} and pronoun binding \citep{monz1998labeled} are more readily modeled and resolved
through automated reasoning and deduction than statistically driven techniques.
\par
Formal methods have historically been the standard means of approaching issues of
natural language understanding, and were supplanted by statistically driven methods
due in large part to issues of computational expense which have since been greatly mitigated \citep{KK_Schubert2015, Gardent2001}.
Now that statistically driven methods which require significant computing resources and collection and annotation of data
comprise many of the standard approaches to modern NLP problems,
there are questions of how to approach these problems when such resources are not available.
Formal methods generally consume less computing space and time than it takes to train neural networks,
and provide a approach to NLP and CL problems in languages where
there is insufficient annotated data for other methods.
Additionally formal methods are inherently explainable,
providing justification for their results and a clearer means of generalization,
since their approach depends on the developer \citep{Basu_2020}.  
This is in contrast to statistically driven methods,
which in a core sense are limited by the training data they are given,
and require more advanced methods to track how they obtain their results \citep{Basu_2020}.  
\par
As a result, use of formal methods in software is critical if one needs to guarantee or discover properties of software systems.
One relevant application is discovery and resolution of bias in NLP systems \citep{gordon-13},
and in general, whenever software systems have access to sensitive resources, one will need to know
and ensure certain things about their behavior before deployment.
As potentially sensitive software systems increasingly involve natural language interfacing
and rely on natural language data (for instance NLP and dialogue systems for health and mission critical tasks),
finding ways to integrate explainability and certainty into approaches to 
natural language tasks will become increasingly relevant.
We hope this thesis will be a contribution to this direction of computational linguistics.   
...
with a corpora of examples, for document level semantics in addition
...
which conveys the illocutionary force of an utterance accompanying the content.
...
In ordinary propositional modal logic writes $\lozenge p$ to indicate that there is a
world/state and another world, where $p$ holds, accessible from that first world.
By contrast, hybrid logic by default writes one of
...
from $a$ specifically.
$a$ in this notation is called a {\it nominal},
a symbol which picks out exactly one world/state in the underlying model.
We also say a nominal $a$ is true at exactly one world or is the
{\it name} of a world (thus the term nominal).
Because of this we can also use the satisfaction operator
to denote that two nominals $i$ and $j$
name the same world and consequently share the same content via:
$$@_i j$$
\par
A {\it world} or {\it state} in a modal or hybrid model for us
is essentially a maximal set of sentences in the language,
in particular, for any proposition $p$ and nominal $a$ we have either
$@_a p$ or $@_a \neg p$, that is, for any proposition it either
holds at the world in question or it does not.  Worlds/states can be used to model any notion
consistent with them, usually different possible states of affairs, for instance
a world identical to ours except New York City is named New Amsterdam,
or another world identical to ours except all cars are green.
We can also use worlds/states to model a world or situation at different {\it times},
for instance everything that exists and is the case right now, exactly thirty five minutes from now, or at 10:23AM UTC May 18th 1997.
Where before we mentioned the notion of whether a world or state was {\it accessible} from
the current world, when states are {\it times}, the question of accesibility
is replaced with two questions, namely whether a given time is before or after
another (or if they are the same time).    
\par
To briefly describe the syntax and semantics of propositional hybrid logic,
...
\item {\it Necessarily, Rabia owns a car or does not own a car.}
  ...
  $$\Box(r \lor \neg r)$$
\end{itemize}
Given our discussion about time, we can use nominals to add the notion of time to these sentences, for instance where
...
AMR concepts such as \texttt{have-rel-role} in first-order language.
...
Where a term $t$ is prefixed by a nominal or state variable $u$
as in $u:t$, this is used to denote the value that $t$ attains specifically
at the time picked out by $u$, since as we will see in the semantics of
{\it FHTL}, each time will have its own domain of entities not necessarily equal
to the domain of any other time, and interpretation of terms is sensitive to
the domain of some particular time.  This prefixing of terms, as in $u:t$, permits
one to specify that a term is to be interpreted in the context of a given time
even if everything else in the formula containing it is to be interpreted in the
context of a different time.
\end{definition}
...
State variables are sent to times/worlds and
first-order variables are sent to $D$, the domain
of the frame.
Thus given a model and a variable assignment $g$,
the interpretation of a term $t$ denoted by $\overline{t}$
is defined by:
\begin{itemize}
\item
...
\begin{figure}
  ...
  \begin{tableau}
    {}
    [@_s (\exists x) {[P((\exists y) {[f(x,y) = f(y,x)]})  \lor \neg (\exists z) {[x = z]}]}, just=Premise
      [@_s P((\exists y) {[f( s_1,y) = f(y, s_1)]})  \lor \neg (\exists z) {[s_1 = z]}, just={\bf ($\exists$)}
        [@_s P( (\exists y) {[ f( s_1,y) = f(y, s_1) ]} ), just={\bf ($\lor$)}
          [@_s P t, just={\bf ($P$)}
            [@_t (\exists y) {[ f( s_1,y) = f(y, s_1) ]}, just={\bf ($P$)}
              [@_t {[ f( s_1,s_2) = f(s_2, s_1) ]}, just={\bf ($\exists$)} ]
            ]
          ]
        ]
        [@_s \neg (\exists z) {[s_1 = z]}, just={\bf ($\lor$)}
          [@_s \neg {[s_1 = s_1]}, just={\bf ($\neg \exists$)} 
            [@_s  {[s_1 = s_1]} , just={\bf (ref)}, close]
          ]
        ]
      ]
    ] 
  \end{tableau}
  \caption{Example {\it FHTL} tableau.}
  \label{tabex}
\end{figure}
...
$s$ not occurring in $\phi$.  For instance in figure \ref{tabex},
we have that the right branch is closed, and that the left branch is
open, since there are no conditions on the function $f$ are provided.
Since this tableau is not closed (it has an open branch), the root formula is satisfiable,
i.e. true in some model, but not necessarily valid.  In the proof of completeness for our
tableau method, we will show how to construct such a model for a tableau with an open branch.
...
$i : t \equiv j : s \Longleftrightarrow @_k i : t = j : s \in \Theta \text{ for some } k \in W$.  From the tableau rules {\bf (ref)}, {\bf (sub)}, and {\bf (:1)}, we observe $\equiv$ is an equivalence
...
is based on the approach in \citet{bohn1998first},
...
We will show later that the new restrictions on the equality
rules, term rules, and the universal quantifier rule given
in figure \ref{restricted}
ensure that eventually any branch will be saturated,
...
\begin{definition}[$\prec_{\Theta}$]
  ...
  is introduced to the branch by an application of
  of {\bf ($F$)} or {\bf ($P$)}
...
\item {\bf Term rules:}
  ...
  conclusion satisfy the premise.
...
\begin{proof}
  ...
  of the form $j: \ldots :i:p$ with a string of at least one distinct prefix,
  ($i$ is called the {\it immediate prefix} in the example) we have the following process.
  Without loss of
  generality, let $i$ be the immediate prefix for the parameter in question below:
  \begin{enumerate}
  \item
...
The meaning in {\it FHTL} is $\llbracket \verb|:op1| \rrbracket \lor \llbracket \verb|:op2| \rrbracket \lor \llbracket \verb|:op3| \rrbracket$.  In the case where the root node of the AMR is \verb|scope|, the order of interpretation is specified.  For instance in example \ref{computer},
...
the interpreted predicate takes the tense operator as an argument.
