...
\begin{doublespacing}
  ...
  \noindent I would like to thank James Pustejovsky for his guidance and patience 
...
While statistically driven methods add a great deal of value to tasks in deep understanding such as dialogue, question answering,
...
Now that statistically driven methods which require significant computing resources and data collection and annotation
...
Formal methods generally consume less computing space and time than neural networks,
and provide a approach to NLP problems for languages where
there is insufficient annotated data for other methods.
Additionally formal methods are inherently explainable,
providing justification for their results,
and provide a more ready means of generalizing their approach
since their approach depends on the developer,
as opposed to statistically driven methods,
...
and in general, anywhere software systems have access to sensitive resource one will need to know
and ensure certain things about their behavior before deployment.
As potentially sensitive software systems increasingly involve natural language interface
and rely on natural language data (NLP and dialogue systems for health and mission critical tasks),
finding ways to integrate explainability and certainty with 
...
which picks out exactly one world in the underlying model.
We also say $a$ is true at exactly one world or is the
{\it name} of the world (thus the term nominal).
...
open, since there are no conditions about the function $f$ available.
Since this tableau is not closed, the root formula is satisfiable
but not valid.
