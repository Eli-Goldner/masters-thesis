...
%%\begin{figure*}[H]
  ...
  \begin{multicols}{3}
    \begin{prooftree}
      \AxiomC{$[s \text{ on  the branch}]$}
      \RightLabel{$[=\textendash\textsf{Ref}]$}
      \UnaryInfC{$@_s t = t$} 
    \end{prooftree}
...
\begin{definition}[{\bf Equality formula}]
  An equality formula of one of either of the following forms:
  \begin{itemize}
  \item
    $@_s t_i = t_j$ where $t_i, t_j$ are terms.
  \item
    $@_n q = @_m q$ where $q$ is a unary function symbol (a non-rigid designator).
  \end{itemize}
\end{definition}
