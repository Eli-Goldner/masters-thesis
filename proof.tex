...
\begin{abstract}
  ...
  ({\it FHL}) and \citeauthor{blackburn2012indexical}'s \citeyearpar{blackburn2012}
  ...
  and {\it BHTL}, which we will call {\it FHTL} (quantified hybrid tense logic),
  and demonstrate {\it FHTL}'s soundess, completeness,
  and decidability. 
...
\begin{alignat*}{2}
  &\mathfrak{M}, g, s \vDash P(t_1, \dotsc , t_n )  && \Longleftrightarrow \langle \overline{t_1}, \dotsc , \overline{t_n} \rangle \in I_s (P)  \\
  &\mathfrak{M}, g, s \vDash t_i = t_j  && \Longleftrightarrow \overline{t_i} = \overline{t_j}  \\
  &\mathfrak{M}, g, s \vDash n && \Longleftrightarrow I_{nom} (n) = s, \text{for }n\text{ a nominal}  \\
  & \mathfrak{M}, g, s \vDash w && \Longleftrightarrow g(w) = s, \text{for }w\text{ a state variable}  \\
  & \mathfrak{M}, g, s \vDash \neg \phi && \Longleftrightarrow  \mathfrak{M}, g, s \not\vDash \phi \\
  & \mathfrak{M}, g, s \vDash \phi \land \psi && \Longleftrightarrow  \mathfrak{M}, g, s \vDash \phi \text{ and } \mathfrak{M}, g, s \vDash \psi \\
  & \mathfrak{M}, g, s \vDash \phi \lor \psi && \Longleftrightarrow  \mathfrak{M}, g, s \vDash \phi \text{ or } \mathfrak{M}, g, s \vDash \psi \\
  & \mathfrak{M}, g, s \vDash \phi \to \psi && \Longleftrightarrow  \mathfrak{M}, g, s \vDash \phi \text{ implies } \mathfrak{M}, g, s \vDash \psi \\
  & \mathfrak{M}, g, s \vDash \exists x \phi && \Longleftrightarrow \mathfrak{M}, g', s \vDash \phi \text{ for some } x\text{-variant }g'\text{ of } g \text{ at } s \\
  & \mathfrak{M}, g, s \vDash \forall x \phi && \Longleftrightarrow \mathfrak{M}, g', s \vDash \phi  \text{ for every } x\text{-variant }g'\text{ of } g \text{ at } s \\
  & \mathfrak{M}, g, s \vDash F \phi && \Longleftrightarrow \mathfrak{M}, g, t \vDash \phi \text{ for some } t \in T \text{ such that } Rst \\
  & \mathfrak{M}, g, s \vDash G \phi && \Longleftrightarrow \mathfrak{M}, g, t \vDash \phi \text{ for all } t \in T \text{ such that } Rst \\
  & \mathfrak{M}, g, s \vDash P \phi && \Longleftrightarrow \mathfrak{M}, g, t \vDash \phi \text{ for some } t \in T \text{ such that } Rts \\
  & \mathfrak{M}, g, s \vDash H \phi && \Longleftrightarrow \mathfrak{M}, g, t \vDash \phi \text{ for all } t \in T \text{ such that } Rts \\
  & \mathfrak{M}, g, s \vDash @_n \phi && \Longleftrightarrow \mathfrak{M}, g, I_{nom}(n) \vDash \phi \text{ for } n \text{ a nominal } \\
  & \mathfrak{M}, g, s \vDash @_w \phi && \Longleftrightarrow \mathfrak{M}, g, g(w) \vDash \phi \text{ for } w \text{ a state variable }
  %%\caption{$AT_x$}
\end{alignat*}
...
\par
In the existential rules \cref{ext}, $p_s$ is a parameter associated with the
...
In the universal rules \cref{univ} $t$ is a grounded term on the branch
which exists at $D_s$.  
...
%%\begin{figure*}[H]
  ...
  \caption{{\it FHTL} Equality rules}
\end{framed}
\end{figure*}
...
\begin{definition}[{\bf Accessibility formula}]
  ...
  of $F$, $P$, $\neg G$, or $\neg H$.
  Additionally we say
  any formula of the form $@_i i$
  is an accessibility formula if it is
  the conclusion of the {\bf nom ref} rule.
\end{definition}
...
While $@_i i$ is not a
modal accessibility formula in the same way as
$@_s F t$ or $@_s P t$ since its meaning does
not involve the accessibility relation $R$
of the model (and modal accessibility relations
need not be reflexive), intuitively every time/world
relates to itself in a similar way to how
different times relate to each other,
especially since we will take for granted that
the collection of times is totally ordered.
The main motivation for considering these
formulae as one kind is that it supports
the termination proof in ways we will soon see.

\begin{definition}[{\bf Equality formula}]
  A formula of the form $@_i t = s$
  or $@_i \neg t = s$ where $s, t$ are extended terms
  is called an equality formula if it is a
  conclusion of {\bf ref}, {\bf :1}, {\bf :2},
  {\bf :3}, {\bf :fix 3}, or {\bf :fix 4}.
\end{definition}
...
\begin{proof}
This is verified by induction on the tableau rules beginning with
$\textit{root}_{\Theta}$ as a base case.
\end{proof}
