...
\begin{figure*}[!h]
  \centering
  \begin{adjustbox}{varwidth=\linewidth,scale=0.8}
    %% \begin{framed}
    \begin{mdframed}
    \centering
    ...
    \begin{multicols}{4}
      \begin{prooftree}
        \AxiomC{$@_s F \phi$}
        \RightLabel{\bf ($F$)\footnote{\label{intro}The nominal $a$ is new to the branch.}}
        \UnaryInfC{$@_s F a$}
        \noLine
        \UnaryInfC{$@_a \phi$}
      \end{prooftree}
      ...
      \begin{prooftree}
        \AxiomC{$@_s P \phi$}
        \RightLabel{\bf ($P$)\footnoteref{intro}}
        \UnaryInfC{$@_s P a$}
        \noLine
        \UnaryInfC{$@_a \phi$}
      \end{prooftree}
...
\begin{definition}[{\bf Quasi-subformula}]
  ...
  with the quantifier and term rules.  
\end{definition}
...
\begin{lemma}[{\bf Decreasing length}]
  ...
  For any nominal $a$ on $\Theta$, $m_{\Theta}(a) \leq |\textit{root}_{\Theta}| - d_{\Theta}(a)$.
  \end{lemma}

\begin{proof}
  The proof is by induction on the depth of $a$.
  For the base case, where $d_{\Theta} (a) = 0$,
  we have that $a$ a is a root nominal and consequently
  $m_{\Theta}(a) \leq |\textit{root}_{\Theta}|$.
  For the inductive step let $b$ be nominal
  such that $d_{\Theta} (b) > 0$.  We need to demonstrate
  $m_{\Theta}(b) \leq |\textit{root}_{\Theta}| - d_{\Theta}(b)$
  assuming the inequality holds for all nominals of lower
  depth.  If there are no root subformulae true at
  $b$ on $\Theta$ then definitionally $m_{\Theta}(b) = - \infty$
  and the inequality holds.  Otherwise where $\phi$ is the earliest
  formula (without loss of generality)
  of maximal length for which $@_b \phi$ is a root subformula
  on $\Theta$, $@_b \phi$ has been introduced by application of an
  {\it FHTL} tableau rule to a root subformula.  The formula $@_b \phi$
  cannot have been introduced to the branch by an
  application of one of the propositional or quantifier rules
  since that would contradict the maximality of $\phi$,
  nor can it have been introduced to the branch by application
  of {\bf (sub)} or any term rules, since then $@_b \phi$ would
  not be the earliest maximal formula.  Assume $@_b \phi$
  has been introduced to the branch by applying {\bf ($@$)}  
  to a root subformula of the form $@_c @_b \phi$, then $b$
  is a root nominal which contradicts the inductive assumption
  and {\bf ($@$)} cannot have been the rule introducing $@_b \phi$,
  similarly if $@_b \phi$ is introduced by {\bf ($\neg @$)}.
  Assume $@_b \phi$ is introduced by {\bf (nom)},
  then it was introduced from premises of the form
  $@_a \phi$, $@_a b$ for some nominal $a$,
  
  
  
  
  \iffalse
  Where $\phi$ is the earliest formula of maximal length
  (term rules that preserve formula type preserve length so we will ignore them)
  true at $t$ on $\Theta$, we need to show
  $m_{\Theta}(s) > |\phi|$.
  Since $s \prec_{\Theta} t$ then $\phi$ must have
  been introduced to the branch by application
  of some {\it FHTL rule}.  However it cannot
  have been introduced to the branch by any of the
  propositional rules or quantifier rules since that would
  contradict the formula's maximality.
  As a result the formula must have been introduced by
  $F$ or $P$, $\neg F$ or $\neg P$, {\bf (nom)}, or an @--rule.
  In the case of $F/P$, $@_t \phi$ must have been introduced to the branch
  from a formula of the form $@_s (F / P) \phi$.
  In the case of $\neg F / \neg P$, $@_s \phi$ must
  be of the form $@_s \neg \psi$ and follows
  from two premises of the form $@_s \neg (F / P) \psi$
  and $@_s (F / P) t$.  In the
  \fi
\end{proof}
