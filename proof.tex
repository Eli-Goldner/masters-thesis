...
\smallskip
The syntax of {\it QHTL} is identical to {\it QHL} except
...
over the collection of nominals/times, we alter the {\it QHL} model definition
to a structure:
$$(T, R, D_t, I_{nom}, I_t)_{t \in T}$$
Identical to the definition for a {\it QHL} model in that:
\begin{itemize}
  \item
...
\item
  $I_t (P) \in D_t^k$ where $P$ is a $k$-ary predicate symbol.
\end{itemize}
...
Where $D = \cup_{t \in T} D_t $, a {\it QHTL} assignment is a function:
$$g : \textsf{SVAR} \cup (\textsf{FVAR} \times T) \to T \cup D $$
...
\begin{itemize}
\item
  $\overline{x} = g_t(x)$ for $x$ a variable and $t \in T$ a time/world.
\item
...
\begin{alignat*}{2}
  &\mathfrak{M}, g, s \dststile{}{} P(t_1, \dotsc , t_n )  && \Longleftrightarrow \langle g_s(t_1), \dotsc , g_s(t_n) \rangle \in I_s (P)  \\
  &\mathfrak{M}, g, s \dststile{}{} t_i = t_j  && \Longleftrightarrow g_s(t_i) = g_s(t_j)  \\
