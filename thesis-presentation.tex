...
\begin{document}
	...
	\end{frame}
        ...
        \begin{frame}{First-Order Hybrid Tense Logic}
          \begin{itemize}
          \item Two main tasks for a sentence $@_s \phi$:
            \begin{itemize}
        ...
        \begin{frame}{Tableau Method}
          ...
          \item
            A tableau caclulus for a logic breaks down and transforms formulae
          \item
        ...
        \begin{frame}{Tableau Proofs}
          ...
          \item
            Completeness is demonstated by contrapositive.
        ...
        \begin{frame}{Tableaux for Model Checking}
          ...
          since their value can depent on the world they are evaluated in,
          i.e. $i:t$, $i:j:t$.
        ...
        \begin{frame}
          ...
          \item
            Restrict term rules to depend on on nominals and terms already on the branch.
        ...
        \begin{frame}{Model Checking Example}
          ...
          $$I_{\textit{tomorrow}}(\texttt{be-located-at-91}) = \{ \langle \textit{computer}_2,  \textit{desk}_2 \rangle, \llbracket \textit{computer}_1,  \textit{desk}_1 \rrbracket \}$$
        ...
        \begin{frame}{Extraction}
          ...
          \texttt{:<aspect> <polarity> :time (<reference> :op1 (<event>))}
          
          \begin{itemize}
          \item
            \texttt{<event>} is some $t \in T$, so formula has the prefix $@_t \phi$
          \item
        ...
        \begin{frame}{Interpretation Example}
          ...
          \small$$\lambda \phi . (\lambda \phi' . (\exists c) (\textit{computer}(c) \land \phi'(c)))(\lambda \psi . (\lambda \psi' . \psi' (\lambda x . \lambda y . \textit{loc}(x,y)))(\lambda \gamma . \phi(\gamma(\psi))) \rhd_{\beta}$$
          \small$$\lambda \phi . (\lambda \phi' . (\exists c) (\textit{computer}(c) \land ((\lambda \psi' . \psi' (\lambda x . \lambda y . \textit{loc}(x,y)))(\lambda \gamma . \phi(\gamma(c))))) \rhd_{\beta}$$
          \small$$\lambda \phi . (\lambda \phi' . (\exists c) (\textit{computer}(c) \land (\phi ( \lambda y . \textit{loc}(c,y))))$$
        \end{frame}
        ...
        \begin{frame}{Conclusion}
          ...
          \item
            Demonstrated how sentences are be handled by these methods.
          \item
