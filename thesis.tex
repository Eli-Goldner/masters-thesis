...
$\textsf{PROP} = \{ \textsf{p}, \textsf{q}, \textsf{r}, \ldots \}$,
modalities $\textsf{MOD} = \{ \textsf{m}, \textsf{m'}, \textsf{m''}, \ldots \}$,
...
which roughly can be seen as renaming $\lozenge$ and $\Box$ to $F$ and $G$ in basic hybrid logic,
and adding their duals under the accessibility relation $P$ and $H$, sometimes written 
$\lozenge^{-1}$ and $\Box^{-1}$
...
uses of $\downarrow$ as in $\downarrow w . \phi$.
...
can be referenced. 
In particular, we have an instance of a local model-checking problem
...
\begin{figure*}[!h]
  \begin{adjustbox}{varwidth=\linewidth,scale=1.0}
    ...
    \begin{mdframed}
      ...
      \begin{prooftree}
        \AxiomC{$@_s \neg (\exists x) \phi$}
        \RightLabel{\bf ($\neg \exists$)\footnote{$p$ is any parameter on the branch and the premise can produce only $Q$ distinct conclusions.}}
        \UnaryInfC{$@_s \neg \phi[s:p / x]$} 
      \end{prooftree}
    \end{mdframed}
  \end{adjustbox}
