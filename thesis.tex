...
The proof of the soundness of the tableaux method
for QHTL is adapted from the proof of
soundness of the tableaux method for
$\mathcal{H}(@)$ given in \citet*{internalizing}.

\bigskip

We can observe from the tableaux rules that
every formula in a tableaux is of the
form  $@_s \phi$ or $\neg @_s \phi$.
We call formulae of these forms
{\it satisfaction statements}.
Give a set of satisfaction statements $\Sigma$
and a tableaux rule $R$ we develop the
