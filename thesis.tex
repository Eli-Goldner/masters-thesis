...
\begin{proof}
  ...
  tableau rules, in some cases it is necessary to
  invoke lemma \ref{subformula}.
\end{proof}
...
\begin{lemma}
  ...
  \begin{itemize}
  \item $@_a \phi \in \Theta \Longrightarrow \mathfrak{M}, [a] \vDash_g \phi \text{ for some (all) variable assignment }g$
  \item $@_a \neg \phi \in \Theta \Longrightarrow \mathfrak{M}, [a] \not \vDash_g \phi \text{ for some (all) variable assignment }g$ 
  \end{itemize}
\end{lemma}
...
\begin{proof}
  ...
  $R [a] [c]$ and $\mathfrak{M}, [c] \vDash_g \psi$.
  ...
  \item {\it $\psi$ a nominal}
    ...
    $R [a] [b]$. By theorem \ref{sim} there is either a root
...
\item {\bf Quantifier rules:}
  ...
  the value of the quantified variable in the premise to the
  domain element assigned to the parameter,
  ...
  As a result we simply check whether each assignment of the quantified
  variable satisfies the premise.
