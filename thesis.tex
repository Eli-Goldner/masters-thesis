...
and $W / \approx$ to denote the set of equivalence classes.  
...
We define $\overline{R}$ as the binary relation on $W / \approx$
...
$D_{[w]}, I_{[w]}$ for $[w] \in W$ and $I_{\textit{nom}}$.
To start we let $D$ be set defined by:
$$D = \{ i : t \mid \text{for some } i \in W \text{ and some closed term of the extended language } t\}$$
