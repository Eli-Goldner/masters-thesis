...
\item {\bf Modal rules:}
  For {\bf ($F$)} and {\bf ($P$)}, that the the premise and the
...
\begin{proof}
  ...
  every possible variable assignment\footnote{Any implementation of this process would represent the totallity of variable assignments via a flag which would consume only constant space, similarly for parameter annotations until we solve for them}.
...
and how to implement this approach.  
